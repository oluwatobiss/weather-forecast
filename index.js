(() => {
  ...
  function getSearchedCity(selectedCity) {
    ...
    async function getWeatherData() {
      try {
        const englishRegionName = new Intl.DisplayNames(["en"], {
          type: "region",
        });
        ...
        searchErrorText.innerText =
          "Oops! That city seems to be nowhere. Try again.";
      }
    }
    getWeatherData();
  }
  ...
  function changeWeatherImageAndIcon(weatherMainDesc) {
    ...
    if (weatherMainDesc === "Thunderstorm") {
      bodyEle.style.backgroundImage =
        'linear-gradient(black, black), url("https://cdn.pixabay.com/photo/2019/11/01/08/44/flashes-4593614_1280.jpg")';
      iconEle.classList.add("fas", "fa-bolt", "weather-icon");
      descriptionIcon.replaceWith(iconEle);
    } else if (weatherMainDesc === "Drizzle") {
      bodyEle.style.backgroundImage =
        'linear-gradient(black, black), url("https://cdn.pixabay.com/photo/2020/04/27/17/58/petals-5100957_1280.jpg")';
      iconEle.classList.add("fas", "fa-cloud-rain", "weather-icon");
      descriptionIcon.replaceWith(iconEle);
    } else if (weatherMainDesc === "Rain") {
      bodyEle.style.backgroundImage =
        'linear-gradient(black, black), url("https://cdn.pixabay.com/photo/2018/07/19/07/03/jesus-3548007_1280.jpg")';
      iconEle.classList.add("fas", "fa-cloud-showers-heavy", "weather-icon");
      descriptionIcon.replaceWith(iconEle);
    } else if (weatherMainDesc === "Snow") {
      bodyEle.style.backgroundImage =
        'linear-gradient(black, black), url("https://cdn.pixabay.com/photo/2019/12/09/17/08/fantasy-4683928_1280.jpg")';
      iconEle.classList.add("fas", "fa-snowflake", "weather-icon");
      descriptionIcon.replaceWith(iconEle);
    } else if (
      weatherMainDesc === "Mist" ||
      weatherMainDesc === "Smoke" ||
      weatherMainDesc === "Haze" ||
      weatherMainDesc === "Dust" ||
      weatherMainDesc === "Fog" ||
      weatherMainDesc === "Sand" ||
      weatherMainDesc === "Ash" ||
      weatherMainDesc === "Squall" ||
      weatherMainDesc === "Tornado"
    ) {
      bodyEle.style.backgroundImage =
        'linear-gradient(black, black), url("https://cdn.pixabay.com/photo/2019/10/07/13/18/mystery-4532583_1280.jpg")';
      iconEle.classList.add("fas", "fa-stream", "weather-icon");
      descriptionIcon.replaceWith(iconEle);
    } else if (weatherMainDesc === "Clear") {
      bodyEle.style.backgroundImage =
        'linear-gradient(black, black), url("https://cdn.pixabay.com/photo/2017/11/03/18/37/sun-2915215_1280.jpg")';
      iconEle.classList.add("fas", "fa-sun", "weather-icon");
      descriptionIcon.replaceWith(iconEle);
    } else if (weatherMainDesc === "Clouds") {
      bodyEle.style.backgroundImage =
        'linear-gradient(black, black), url("https://cdn.pixabay.com/photo/2016/11/18/01/45/moon-1833172_1280.jpg")';
  ...
  async function getTimeAndDate(cityAndCountry) {
    try {
      const timeAndDateData = await fetch(
        `https://timezone.abstractapi.com/v1/current_time?api_key=6c6db9bc95f54f6f99ffb4b7a1c892b1&location=${cityAndCountry}`
      );
